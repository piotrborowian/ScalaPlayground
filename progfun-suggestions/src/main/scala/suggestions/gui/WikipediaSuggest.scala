...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered {
      term => ObservableEx(wikipediaSuggestion(term)).timedOut(1)
    }

    // TO IMPLEMENT
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe { newTerm => 
      newTerm match {
    ...
    val selections: Observable[String] = {
      button.clicks.flatMap { b =>
      	Observable(suggestionList.selection.items.toList:_*)
      }
    }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered {
      page=> wikiPageResponseStream(page)	
    }

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe { page => 
      page match {
      	case Success(content) => editorpane.text = content
      	case Failure(error)   => editorpane.text = error.getMessage	
      }
    }
  }
