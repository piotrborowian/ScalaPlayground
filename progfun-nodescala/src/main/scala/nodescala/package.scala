...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      fs.foldRight(promise.success(Nil).future) { 
      	case (f, acc) => for {
      	  elem <- f
      	  accList <- acc
      	} yield elem::accList	
    ...
    def delay(t: Duration): Future[Unit] = {
      future {blocking(Thread.sleep(t.toMillis))}
    }

    def delay[T](v : T, t : Duration) : Future[T] = {
      val promise = Promise[T]()
      delay(t).onComplete { _ =>
      	promise.success(v)
      }	
      promise.future 
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
     ...
     val future = f(tokenSource.cancellationToken) //start the computation
     tokenSource
    }
