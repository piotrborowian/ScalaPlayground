...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
  	while (response.hasNext && !token.isCancelled) {
  	  val responseNextPart = response.next
  	  exchange.write(responseNextPart)
  	}
  	exchange.close()
  }
  ...
  def start(relativePath: String) (handler: Request => Response): Subscription = {
  	...
  	val processRequestsSubscription = Future.run() { ct =>
  	  future {
  	    while (ct.nonCancelled) {
  	      val (request, exchange) = blocking {
  	      	Await.result((listener.nextRequest), Duration.Inf)
  	      }
  	      future {
  	...
  	Subscription(processRequestsSubscription, listenerSubscription)
  }
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      createContext { exchange =>
      	val futureValue = (exchange.request, exchange) 
      	promise.success(futureValue)
      	removeContext
      }
